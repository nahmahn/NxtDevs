...
export default function PracticeSession() {
    ...
    useEffect(() => {
        const fetchFilters = async () => {
            try {
                const res = await fetch("http://localhost:8000/api/v1/questions/filters");
                if (res.ok) {
    ...
    const fetchQuestion = useCallback(async () => {
        ...
        try {
            ...
            const res = await fetch(`http://localhost:8000/api/v1/questions/next?${queryParams}`, {
    ...
    const handleSubmit = async (optionId: string) => {
        ...
        try {
            const userId = localStorage.getItem('user_id');
            const res = await fetch(`http://localhost:8000/api/v1/questions/${question.id}/submit`, {
                method: "POST",
                headers: {
