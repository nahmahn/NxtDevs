...
export default function DuelPage() {
    ...
    const fetchDuelState = async () => {
        ...
        try {
            const res = await fetch(`http://localhost:8000/api/v1/duel/${currentDuelId}`, {
    ...
    const joinQueue = async () => {
        ...
        setError(null);
        try {
            const res = await fetch("http://localhost:8000/api/v1/duel/join-queue", {
                method: "POST",
    ...
    const leaveQueue = async () => {
        try {
            await fetch("http://localhost:8000/api/v1/duel/leave-queue", {
                method: "POST",
                headers: { "X-User-Id": currentUserId || '' }
            });
        } catch (e) {
    ...
    const submitAnswer = async (optionId: string) => {
        ...
        try {
            const res = await fetch(
                `http://localhost:8000/api/v1/duel/${duelId}/answer?option_id=${optionId}&time_ms=${timeTaken}`,
                {
                    method: "POST",
                    headers: { "X-User-Id": currentUserId || '' }
                }
